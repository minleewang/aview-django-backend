...
class GuestAccountController(viewsets.ViewSet):
    __guestAccountService = GuestAccountServiceImpl.getInstance()
    ...
    def requestEmail(self, request):
        ...
        try:
            # Redis에서 userToken에 해당하는 guestAccountId 가져옴
            guestAccountId = self.redisCacheService.getValueByKey(userToken)

            if not guestAccountId:
                # Redis에서 guestAccountId 찾지 못한 경우
                return JsonResponse({"error": "유효한 userToken이 아닙니다", "success": False}, status=status.HTTP_404_NOT_FOUND)

            # accountId를 사용하여 이메일을 찾음
            foundEmail = self.__guestAccountService.findEmail(guestAccountId)
